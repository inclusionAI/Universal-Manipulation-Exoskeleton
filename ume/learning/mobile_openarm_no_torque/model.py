...
class ACT_Abs(nn.Module):
    ...
    def build_model_and_optimizer(cls, n_obs_steps, action_chunk_size, seed):
        ...
        obs_dim = 16      # openarm joint state: 16 joints (no torque)
        action_dim = 19   # openarm action: 19 joints
        hidden_dim = 256
        camera_names = []
    ...
    def get_obs(cls, batch_data):
        ...
        qpos = torch.cat([
            # batch_data["obs"]["robot0_eef_pos"], 
            # batch_data["obs"]["robot0_eef_quat"], 
            # batch_data["obs"]["robot0_gripper_qpos"]
            batch_data["obs"]["joint_state"]
        ], dim=-1)  # (B, n_obs_steps, 8)
        return img, qpos

    def get_action(cls, batch_data):
        return batch_data["action"]  # (B, action_chunk_size, action_dim)
    ...
    def predict_action(self, batch_data):
        img, qpos = self.get_obs(dict(obs=batch_data))
        normalized_img = self.img_normalizer.normalize(img)
        normalized_qpos = self.state_normalizer.normalize(qpos)
        normalized_qpos = normalized_qpos.view(normalized_qpos.shape[0], -1)  # (B, n_obs_steps*state_dim)
        obs_lowdim = self.obs_lowdim_map(normalized_qpos)  # (B, hidden_dim)
        ...
        return dict(
            action=a_hat,
            openarm_left_action=openarm_left_action,
            openarm_right_action=openarm_right_action,
...
if __name__ == "__main__":
    ...
    act_model, optimizer = ACT_Abs.build_model_and_optimizer(
        n_obs_steps=2,
